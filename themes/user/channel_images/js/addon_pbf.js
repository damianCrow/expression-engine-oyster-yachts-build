...
ChannelImages.Init = function(){
    ...
    // Submit Entry Stop
    var form = ChannelImages.CFields.closest('form');
    ...
    form.bind('submit', function(evt){
        ...
        if (ChannelImages.CFields.find('.ImageQueue div.Done').length > 0){
            var submitBtn = $(evt.target).find('.form-ctrls').find('input[type=submit]');
            submitBtn.parent(':first').append('<div class="ChannelImagesSubmitWait">' + ChannelImages.LANG.submitwait + '</div>');
        }
    });
...
ChannelImages.AddNewFile = function(JSONOBJ, FIELD_ID, Sync){
    ...
    if (!JSONOBJ.url_title) JSONOBJ.url_title = '';
...
ChannelImages.OpenImageEdit = function(e){
    ...
    $.post(ChannelImages.AJAX_URL+'&ajax_method=edit_image_ui', PostParams, function(data){
        ...
        Elem.find('.bottombar').on( 'click','.save_image', ChannelImages.EditImageSave);
        Elem.find('.bottombar').on( 'click','.cancel_image', function(e){
    ...
    $.post(ChannelImages.AJAX_URL, Params, function(rData){
        ...
        $(e.target).closest('table').find('.ImgUrl img').each(function(i, elem){
                    ...
                    if ( Params.entry_id > 0 ){
                        if ($('#locationtype').val() == 'local') {
                           $(elem).attr('src', $(elem).attr('src')+'?'+d.getTime()+'&'+d.getTime() );
                         } else {
                               $(elem).attr('src', $(elem).attr('src')+'&'+d.getTime()+'&'+d.getTime() );
                         }
                    }else {
