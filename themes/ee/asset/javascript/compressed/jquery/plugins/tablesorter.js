...
a.normalized.push(f),f=null}return r.config.debug&&e("Building cache for "+n+" rows:",i),a}function d(e,r){if(!r)return"";var i="";return i="simple"==e.textExtraction?r.childNodes[0]&&r.childNodes[0].hasChildNodes()?r.childNodes[0].innerHTML:r.innerHTML:"function"==typeof e.textExtraction?e.textExtraction(r):t(r).text()}function c(r,i){if(r.config.debug)var n=new Date;for(var o=i,s=o.row,a=o.normalized,d=a.length,c=a[0].length-1,u=t(r.tBodies[0]),f=[],l=0;d>l;l++)if(f.push(s[a[l][c]]),!r.config.appender)for(var g=s[a[l][c]],m=g.length,p=0;m>p;p++)u[0].appendChild(g[p]);r.config.appender&&r.config.appender(r,f),f=null,r.config.debug&&e("Rebuilt table:",n),h(r),setTimeout(function(){t(r).trigger("sortEnd")},0)}function u(i){if(i.config.debug)var n=new Date;for(var o=(!!t.metadata,[]),s=0;s<i.tHead.rows.length;s++)o[s]=0;return $tableHeaders=t("thead th",i),$tableHeaders.each(function(e){this.count=0,this.column=e,this.order=m(i.config.sortInitialOrder),(f(this)||l(i,e))&&(this.sortDisabled=!0),this.sortDisabled||t(this).addClass(i.config.cssHeader),
// add cell to headerList
i.config.headerList[e]=this}),i.config.debug&&(e("Built headers:",n),r($tableHeaders)),$tableHeaders}function f(e){return!(!t.metadata||t(e).metadata().sorter!==!1)}function l(t,e){return!(!t.config.headers[e]||t.config.headers[e].sorter!==!1)}function h(t){for(var e=t.config.widgets,r=e.length,i=0;r>i;i++)g(e[i]).format(t)}function g(t){for(var e=B.length,r=0;e>r;r++)if(B[r].id.toLowerCase()==t.toLowerCase())return B[r]}function m(t){return"Number"!=typeof t?i="desc"==t.toLowerCase()?1:0:i=1==t?t:0,i}function p(t,e){for(var r=e.length,i=0;r>i;i++)if(e[i][0]==t)return!0;return!1}function b(e,r,i,n){
