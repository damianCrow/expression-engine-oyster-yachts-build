...
class Low_reorder extends Low_reorder_base {
	...
	private function _get_set($set_id)
	{
		...
		$sets = array_filter((array) low_get_cache($this->package, 'sets'));
		$set  = array();
		...
		if (empty($set))
		{
			...
			low_set_cache($this->package, 'sets', $sets);
		}
	...
	private function _filter_entry_ids()
	{
		...
		$entries = array_filter((array) low_get_cache($this->package, 'entry_ids'));
		...
		if ( ! isset($entries[$key]))
		{
			...
			low_set_cache($this->package, 'entry_ids', $entries);
	...
	{
		// Get entries from cache
		$entries = array_filter((array) low_get_cache($this->package, 'entries'));
		...
		if ( ! isset($entries[$url_title]))
		{
			...
			// Register new sets array to cache
			low_set_cache($this->package, 'entries', $entries);
		}
		else
		{
	...
	private function _get_cat_id($cat_groups = array())
	{
			...
			($cat_word = ee()->config->item('reserved_category_word')) != '')
		{
			...
			{
				// Get category cache
				$categories = (array) low_get_cache($this->package, 'categories');
				...
				if ( ! ($cat_id = (int) array_search($cat_url_title, $categories)))
				{
					...
					low_set_cache($this->package, 'categories', $categories);
				}
				else
				{
