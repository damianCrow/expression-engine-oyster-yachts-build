...
class ImageAction_flip extends ImageAction
{
	...
	public function run($file)
	{
		...
		for ($x=0 ; $x<$width ; $x++)
		{
			for ($y=0 ; $y<$height ; $y++)
			{
				if ($this->settings['axis'] == 'both') imagecopy($imgdest, self::$imageResource, $width-$x-1, $height-$y-1, $x, $y, 1, 1);
				else if ($this->settings['axis'] == 'horizontal') imagecopy($imgdest, self::$imageResource, $width-$x-1, $y, $x, $y, 1, 1);
				else if ($this->settings['axis'] == 'vertical') imagecopy($imgdest,self::$imageResource, $x, $height-$y-1, $x, $y, 1, 1);
			}
		}
