...
class ImageAction_border extends ImageAction
{
	...
	public function run($file, $temp_dir)
	{
		...
		$res = $this->open_image($file);
              /*  echo 'File border '.$file."  -- ".$temp_dir."<br>";*/

		if ($res != TRUE) return FALSE;

		$border_width = $this->settings['thickness'];
		$border_color = $this->settings['color'];
		$width_old = self::$imageResource_dim['width'];
		$height_old = self::$imageResource_dim['height'];

		$old = imagecreatetruecolor($width_old, $height_old);
		imagecopy($old,self::$imageResource, 0, 0, 0, 0, $width_old, $height_old);
		...
		self::$imageResource = imagecreatetruecolor($width, $height);
		imagealphablending(self::$imageResource, true);
		imagesavealpha(self::$imageResource, false);

		$color = $this->hex_to_rgb( $border_color, 'fff' );
		$border_color = imagecolorallocate(self::$imageResource, $color[0], $color[1], $color[2]);

		//top border
		imagefilledrectangle( self::$imageResource, 0, 0, $width, $border_width, $border_color );
		//right border
		imagefilledrectangle( self::$imageResource, $width_old + $border_width, $border_width, $width, $height_old + $border_width, $border_color );
		//bottom border
		imagefilledrectangle( self::$imageResource, 0, $height_old + $border_width, $width, $height, $border_color );
		//left border
		imagefilledrectangle( self::$imageResource, 0, $border_width, $border_width, $height_old + $border_width, $border_color );
		//copy image over top
		imagecopyresized( self::$imageResource, $old, $border_width, $border_width, 0, 0, $width_old, $height_old, $width_old, $height_old );
