...
class Channel_images_model
{
	...
	public function get_images($entry_id=FALSE, $field_id=FALSE, $params=array(), $tagdata='')
	{
		...
		if (strpos($tagdata, LD.'/'."{$this->prefix}paginate".RD) === FALSE) ee()->db->limit($limit);
		...
		if (isset($params['cover_only']) === FALSE)
		{
			...
			if ($params['orderby'] == 'title') ee()->db->order_by('title', $sort);
			if ($params['orderby'] == 'date') ee()->db->order_by('upload_date', $sort);
			elseif ($params['orderby'] == 'random') ee()->db->order_by('RAND()', FALSE);
			else ee()->db->order_by('image_order', $sort);
		}
		...
		if (isset($params['channel']) === TRUE)
		{
			...
			if (is_array($cid) === TRUE) ee()->db->where_in('channel_id', $cid);
			else ee()->db->where('channel_id', $cid);
		}
	...
	public function parse_template($entry_id=FALSE, $field_id=FALSE, $params=array(), $tagdata)
	{
		...
		if ( preg_match_all( "/".LD."({$this->prefix}switch\s*=.+?)".RD."/is", $tagdata, $this->switch_matches ) > 0 )
		{
			...
			{
				$this->switch_vars[$key] = ee()->functions->assign_parameters($this->switch_matches[1][$key]);
	...
	public function parse_single_image_row($count, $image, $tagdata)
	{
		...
		$settings = $this->get_field_settings($image->field_id);
                if (isset($settings['settings'])==true) $settings=array_merge($settings,$settings['settings']);
	...
	public function get_field_settings($field_id)
	{
		if (isset($this->session->cache['channel_images']['field_settings'][$field_id]) == FALSE)
		{
			...
			//$this->session->cache['channel_images']['field_settings'][$field_id] = unserialize(base64_decode($query->row('field_settings')));
                        ee()->cache->save('channel_images/field_settings/'.$field_id,unserialize(base64_decode($query->row('field_settings'))),500);
	...
	public function get_channel_id($channels)
	{
		...
		if (strpos($channels, '|') !== FALSE)
		{
			...
			foreach ($channels as $key => $value)
			{
				...
				{
					$return[] = ee()->session->cache['devdemon']['channel_to_id'][$value];
					continue;
				}
			...
			if (empty($lookup) === FALSE)
			{
				...
				{
					ee()->session->cache['devdemon']['channel_to_id'][$row->channel_name] = $row->channel_id;
		...
		if (isset($params['field']) === TRUE)
		{
			...
			if (strpos($params['field'], '|') !== FALSE)
			{
				...
				foreach($pfields as $field)
				{
					if (isset($this->EE->session->cache['channel']['custom_channel_fields'][$site_id][ $field ]) === FALSE)
					{
						...
						if ($query->num_rows() == 0)
						{
							if (isset(ee()->TMPL) === TRUE) ee()->TMPL->log_item('CHANNEL_IMAGES: Could not find field : ' . $field);
							return FALSE;
						}
						else
						{
			...
			else
			{
				if (isset($this->EE->session->cache['channel']['custom_channel_fields'][$site_id][ $params['field'] ]) === FALSE)
				{
					...
					if ($query->num_rows() == 0)
					{
						if (isset(ee()->TMPL) === TRUE) ee()->TMPL->log_item('CHANNEL_IMAGES: Could not find field : ' . $params['field']);
						return FALSE;
					}
					else
					{
