    ...
    public function test_location()
    {
        ...
        if ($this->init() == FALSE) {
            exit('<div class="alert inline issue">
                CLOUDFILES INIT FAILED. <br /> Check USERNAME, API KEY, CONTAINER?
            </div>');
        }
        ...
        {
            $this->CF_CONT = $this->CF_CONN->get_container($this->lsettings['container']);
            $o .= '<li class="act">Container Exists?: <span class="good">Yes</span></li>';
        }
        catch (NoSuchContainerException $e)
        {
            $o .= '<li class="remove">Container Exists: <span class="bad">No</span></li>';

            // Create Container
            try {
                $this->CF_CONT = $this->CF_CONN->create_container($this->lsettings['container']);
                $this->CF_CONT->make_public();
                $o .= '<li class="act">Container Creation: ' . '<span class="good">Passed</span></li>';
            } catch (Exception $e) {
                $o .= '<li class="remove">Container Creation: ' . '<span class="bad">Failed</span> <br>';
                $o .= '<em>' . $e->getMessage() . '</em></li>';
                exit($o);
            }
        }
        catch (Exception $e)
        ...
        try
        {
            ...
            $o .= '<li class="act">Write Test File: ' . '<span class="good">Passed</span></li>';
        }
        catch (Exception $e)
        {
            $o .= '<li class="remove">Write Test File: ' . '<span class="bad">Failed</span> <br>';
            $o .= '<em>' . $e->getMessage() . '</em></li>';
            exit($o);
        }
