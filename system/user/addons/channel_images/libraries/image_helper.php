 ...
 */
 class channel_images_std extends stdClass{
...
class Image_helper
{
	...
	{
		// Creat EE Instance
		$this->channel_images=  new stdClass();// new channel_images_std();
		$this->site_id = ee()->config->item('site_id');
       
		if (isset($this->channel_images) === false) {           
		                 
			$this->channel_images->config = ee()->config->item('channel_images');
		}
	...
	function define_theme_url()
	{
		...
		{
			$theme_url = $this->EE->config->item('theme_folder_url').'user/';
		}
	...
	{
		$theme_url = trim(ee()->config->item('theme_folder_url').'user/' . $this->package . '/');
	...
	function grab_settings($site_id=FALSE)
	{
		...
		else
		{
			...
			ee()->db->where('module_name', 'Channel_images');
	...
	function get_upload_preferences($group_id = NULL, $id = NULL, $ignore_site_id = FALSE)
	{
		...
		if (version_compare(APP_VER, '2.1.5', '>='))
		{
			...
			if ($group_id == 1)
			{
				...
				if ($ignore_site_id != TRUE) ee()->db->where('site_id', ee()->config->item('site_id'));
				ee()->db->order_by('name');
	...
	{
		$default_settings = ee()->config->item('ci_defaults');
		...
		if (isset($this->EE->session->cache['channel_images']['field_settings'][$field_id]) === false) {
			...
			// Any overrides?
			if (isset($this->channel_images->config['fields'][$field_id]) === true && is_array($this->channel_images->config['fields'][$field_id]) === true) {
				$settings = $this->array_extend($settings, $this->channel_images->config['fields'][$field_id]);
				$settings['override'] = $this->channel_images->config['fields'][$field_id];
			}
	...
	function get_router_url($type='url', $method='channel_images_router')
	{
		...
		if (isset($this->EE->session->cache['Channel_Images']['Router_Url'][$method]['ACT_ID']) == FALSE)
		{
			...
			ee()->db->where('class', 'Channel_images');
		...
		if ($type == 'url')
		{
			...
			$over = ee()->config->item('channel_images_act_url');
	...
	public function decode_json($obj, $return_array=false)
	{
