		...
		<table cellspacing="0" cellpadding="0" class="CITable"> <thead> <tr>
			...
			<?php foreach($row as $image):?>
				...
				<tr>
					...
					<td><a href='<?=$image->big_img_url?>' class='ImgUrl' rel='CISearchResult' title="<?=$image->title?>"><img src='<?=$image->small_img_url?>' width='<?=ee('channel_images:Settings')->settings['image_preview_size']?>'/></a></td>
					<td><?=$image->title?></td>
