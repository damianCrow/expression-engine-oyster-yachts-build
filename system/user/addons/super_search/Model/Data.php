...
class Data extends AddonBuilder
{
	...
	public function set_default_cache_prefs()
    {
	...
	function build_lexicon($type = 'single', $entry_id = 0, $batch = 0, $total = 50)
	{
		...
		if ($type == 'build' AND $batch == 0)
		{
			...
			ee()->db->update('exp_super_search_terms', array('entry_count' => 0));
		}
		...
		if ($type == 'build' OR $type == 'rebuild')
		{
			...
			if ($entries->count() > 0)
			{
				...
				{
					$this->sess['lexicon']['entry_ids'][] = $entry->entry_id;
				}
			}
			else
			{
		...
		$new = $this->make('LexiconLog');
		$new->type			= 'build';
		$new->entry_ids		= $entries_str;
		$new->origin		= 'manual';
		$new->action_date	= ee()->localize->now;
		$new->save();
	...
	function _load_fieldtypes()
	{
		if (! isset($this->sess['lexicon']['fields']))
		{
			...
			if ($fields->count() > 0)
			{
				...
				{
					$this->sess['lexicon']['fields']['searchable'][] = 'field_id_' . $field->field_id;

					// Include the file if it doesn't yet exist
					if (! class_exists($fieldtypes[$field->field_type]['class']) AND file_exists($fieldtypes[$field->field_type]['path'] . $fieldtypes[$field->field_type]['file']))
					{
						require $fieldtypes[$field->field_type]['path'] . $fieldtypes[$field->field_type]['file'];
					}

					// Only initiate the fieldtypes that have the necessary method
					if (method_exists($fieldtypes[$field->field_type]['class'],'low_search_index')
						OR method_exists($fieldtypes[$field->field_type]['class'],'third_party_search_index'))
					{
						$this->sess['index']['fields']['field_id_'.$field->field_id] = $fieldtypes[$field->field_type];
			...
			if ($third_party_fields)
			{
				...
				if (count($pref_cols) > 0)
				{
					...
					$pref = $this->fetch('Preference')
						->fields('preference_value')
						->filter('site_id', ee()->config->item('site_id'))
						->filter('preference_name', 'third_party_search_indexes')
						->first();

					$pref->preference_value	.= trim("|" . implode($pref_cols, "|"), '|');
					$pref->save();
				}
			}
		}
	...
	public function define_levenshtein( )
	{
		$sql = "
