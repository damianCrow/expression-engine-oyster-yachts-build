...
class Zenbu_cartthrob_price_simple_ft extends Base
{
	/**
	*	Constructor
	*
	...
	*/
	public function zenbu_display($entry_id, $channel_id, $data, $table_data = array(), $field_id, $settings, $rules = array(), $upload_prefs = array(), $installed_addons)
	{
		if( ! empty($data))
		{
	...
	*/
	public function zenbu_get_table_data($entry_ids, $field_ids, $channel_id, $output_upload_prefs, $settings)
	{
		$output = array();
		ee()->db->from('cartthrob_settings');
		ee()->db->where('key', 'number_format_defaults_prefix');
